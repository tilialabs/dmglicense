...
void dump_rsrc_file(const char *type, NSData *data, NSString *rPath)
{
	...
	switch (locale) {
        case 5000:
			fprintf(fd, "data '%s' (5000, \"English\") {\n", type);
			break;
		case 5001:
			fprintf(fd, "data '%s' (5001, \"German\") {\n", type);
			break;
		case 5002:
			fprintf(fd, "data '%s' (5002, \"Spanish\") {\n", type);
			break;
		case 5003:
			fprintf(fd, "data '%s' (5003, \"French\") {\n", type);
			break;
		case 5004:
			fprintf(fd, "data '%s' (5004, \"Italian\") {\n", type);
			break;
		case 5005:
			fprintf(fd, "data '%s' (5005, \"Japanese\") {\n", type);
			break;
		case 5006:
			fprintf(fd, "data '%s' (5006, \"Dutch\") {\n", type);
			break;
		case 5007:
			fprintf(fd, "data '%s' (5007, \"Swedish\") {\n", type);
			break;
		case 5008:
			fprintf(fd, "data '%s' (5008, \"Brazilian Portuguese\") {\n", type);
			break;
		case 5009:
			fprintf(fd, "data '%s' (5009, \"Simplified Chinese\") {\n", type);
			break;
		case 5010:
			fprintf(fd, "data '%s' (5010, \"Traditional Chinese\") {\n", type);
			break;
		case 5011:
			fprintf(fd, "data '%s' (5011, \"Danish\") {\n", type);
			break;
		case 5012:
			fprintf(fd, "data '%s' (5012, \"Finnish\") {\n", type);
			break;
		case 5013:
			fprintf(fd, "data '%s' (5013, \"French Canadian\") {\n", type);
			break;
		case 5014:
			fprintf(fd, "data '%s' (5014, \"Korean\") {\n", type);
			break;
		case 5015:
			fprintf(fd, "data '%s' (5015, \"Norwegian\") {\n", type);
			break;
		default:
			fprintf(fd, "data '%s' (5000, \"English\") {\n", type);
			break;
...
void rtf2r(const char *filename)
{
	...
	//
		...
		for( ; index < count; ++index ) {
				...
				rtfs.StartIndex = NSSwapHostIntToBig(rtfs.StartIndex);
